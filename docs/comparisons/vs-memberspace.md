...
| Hosting | Included | Separate (need existing website + hosting) |
| AI customization | $9–29/mo (optional) | Not available |
| **Typical cost (100 members, $20/mo dues)** | **~$14/mo** | **$29/mo + $100/mo in transaction fees = $129/mo** |
...
| Content gating | Feature-flagged access control | Yes (core feature — gate any page) |
| Membership tiers | Yes (with management tools) | Yes (multiple plans) |
| Recurring billing | Not yet (external Stripe link) | Yes (Stripe integration) |
| Event management | Yes | No |
| Announcements | Yes | No |
| Forum / discussions | Yes | No |
| Admin dashboard | Yes (full analytics) | Basic (member list, revenue) |
| Inline editing | Yes | N/A (uses host site's editor) |
| AI customization | Yes | No |
| Works with existing site | No (standalone portal) | Yes (core value prop — bolts onto any site) |
| Custom design | Full HTML/CSS | Depends on host site |
| Self-hosted / own data | Yes | Partially (your site, their auth layer) |
| i18n / multi-language | Yes | No |
| Community features | Yes (directory, forum, feed) | None |
