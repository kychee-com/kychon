...
| AI customization | Yes (Lychee Pro) | No (but plugins exist) |
| Self-hosted | Yes | Yes |
| Open source | Yes | Yes |
...
- **Real-time** — Live updates, user presence, typing indicators. Wild Lychee is HTTP-only (no WebSocket).
- **Email integration** — Full mailing list mode. Users can participate entirely via email. Wild Lychee has basic notifications.
- **Plugin ecosystem** — Mature plugin system with community contributions. Wild Lychee's extensibility is via AI agent.
...
- **Moderation tools** — Sophisticated moderation, flagging, trust levels, auto-moderation. Wild Lychee has basic moderation.
- **Scale** — Handles very large communities (100k+ users) with proven performance.

## Where Wild Lychee wins

- **Membership management** — Wild Lychee is built for organizations that manage members: tiers, approvals, renewals, expiration tracking, directory. Discourse is built for conversations.
...
- **Simplicity** — Wild Lychee is purpose-built and lightweight. Discourse is powerful but complex to configure and maintain (Ruby on Rails, Redis, PostgreSQL, Sidekiq).
