...
Kychon internationalization provides runtime translation lookup, plural handling, cached locale loading, language picker visibility, and RTL support so portals can render localized UI consistently.

## Requirements

### Requirement: Astro i18n routing for locale-prefixed URLs

The Astro config SHALL define i18n routing with `prefixDefaultLocale: false`. The default locale (English) SHALL use unprefixed URLs such as `/events.html`. Other locales SHALL be represented by configured locale metadata and may use prefixed URLs such as `/es/events.html` where generated by the project.

#### Scenario: English URLs have no prefix
- **WHEN** a user visits `/events.html`
- **THEN** the page loads using the default English locale unless a saved locale preference overrides it

#### Scenario: Locale preference selects translations
- **WHEN** a user selects Spanish from the portal language controls
- **THEN** the locale is saved
- **AND** `t()` returns Spanish translations on subsequent page loads and transitions

### Requirement: t() translation function

`src/lib/i18n.ts` SHALL export an ES module `t(key, vars)` function. It SHALL return the translated string for the current locale, falling back to the English string if the key is missing. Interpolation SHALL replace `{placeholder}` with values from the `vars` object.
