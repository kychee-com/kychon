...
describe('admin action controls source', () => {
  ...
  it('routes simple text edits through a shared shadcn prompt island', async () => {
    ...
    expect(editor).toContain("adminEditableEventTarget(event, 'data-editable')");
    expect(editor).toContain('nearestElementWithAttribute');
    expect(editor).toContain('nearestElementWithTagName');
    expect(editor).not.toContain('.closest(');
    expect(editor).not.toContain('getElementById');
  ...
  it('routes admin action buttons through a single delegated click handler', async () => {
    ...
    expect(editor).toContain("adminActionEventTarget(event, 'data-section-remove')");
    expect(editor).toContain("adminActionEventTarget(event, 'data-scope-toggle')");
    expect(editor).toContain("adminActionEventTarget(event, 'data-zone-add')");
    expect(editor).toContain("adminActionEventTarget(event, 'data-section-edit')");
    expect(editor).toContain("adminActionEventTarget(event, 'data-nav-edit')");
    expect(editor).toContain("adminActionEventTarget(event, 'data-embed-edit')");
    expect(editor).not.toContain('.closest(');
    expect(editor).not.toContain('getElementById');
    expect(editor).toContain('initAdminActionDelegates();');
