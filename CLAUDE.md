...
- **Block registry** (`src/lib/blocks.ts`) — single isomorphic `renderBlock(section, ctx): string` runs at Astro build time (Node) and at runtime (browser). Dynamic blocks emit a skeleton at bake time and have a `hydrate(el, section, ctx)` that fetches data and replaces the body. Block types include `hero`, `features`, `cta`, `stats`, `testimonials`, `faq`, `polls`, `event_countdown`, `announcements_feed`, `activity_feed`, `nav`, `brand_header`, `sign_in_bar`, `footer_address`, `footer_links`, `footer_copyright`, `footer_social`, `footer_attribution`.
...
- **Build-time bake** — `Portal.astro`'s frontmatter calls `renderBlock` against the active project's `zone='header'` and `zone='footer'` blocks at build time. The result is injected into `#zone-header` and `#zone-footer` containers via `set:html`, giving instant chrome on cold visits with no flicker.
...
├── public/
│   ├── css/               # Global CSS (theme.css, styles.css, a11y.css, admin-editing.css)
